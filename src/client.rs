use std::{net::UdpSocket, sync::{Arc, Mutex, mpsc::{Sender, Receiver}}, fs::File, io::{Seek, Write, SeekFrom}};

use tracing::{error, warn, info};

use crate::{MTU, protocol::Message, PART_SIZE, BUF_CAPACITY};
                ...
                if part_buffer.len() == part_buffer.capacity() || parts_received == nb_parts {
                    ...
                    for i in 0..(part_buffer.len()-1) {
                        ...
                        if slab.len() == slab.capacity() {
                            file.seek(SeekFrom::Start(cursor)).expect("Could not seek in file");
...
fn handle_client_sync(socket: UdpSocket, sync_chan: Receiver<Sync>) {
    loop {
        match sync_chan.recv() {
            Ok(sync) => {
                if sync.ack.len() > 0 {
                    socket.send(&Message::Ack { ids: sync.ack }.serialize()).expect("Could not send ACK");
                }
                if sync.loss.len() > 0 {
                    socket.send(&Message::Loss { ids: sync.loss }.serialize()).expect("Could not send LOSS");
                }
            },
            Err(err) => {
...
fn handle_client_read(socket: UdpSocket, nb_parts: u32, parts_received: Arc<Mutex<Vec<u32>>>, file_chan: Sender<(u32, Vec<u8>)>, sync_chan: Sender<Sync>) {
            ...
            Ok(size) => {
                ...
                match Message::parse(data) {
                    Ok(Message::Part { id, data }) => {
                        file_chan.send((id, data)).expect("Could not send chunk to writer.");
                        {
                            let mut _guard = parts_received.lock().expect("Could not lock ack array");
                            _guard.push(id);
                            _guard.sort();
                    ...
                    Ok(Message::Sync { ids }) => {
                        ...
                        for id in ids {
                            if parts_received.lock().expect("Could not lock ack array").binary_search(&id).is_err() {
                        ...
                        sync_chan.send(Sync::new(ack, loss)).expect("Could not trigger sync.");
                    }
                    Ok(msg) => {
